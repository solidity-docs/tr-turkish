...
Parser::FunctionCallArguments Parser::parseNamedArguments()
{
	...
	while (m_scanner->currentToken() != Token::RBrace)
	{
		...
		// Add name
		ret.parameterNames.push_back(identWithLocation.first);
		// Add location
		ret.parameterNameLocations.push_back(identWithLocation.second);

		expectToken(Token::Colon);
		ret.arguments.push_back(parseExpression());
